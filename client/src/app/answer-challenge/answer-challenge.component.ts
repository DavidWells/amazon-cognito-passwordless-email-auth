...
export class AnswerChallengeComponent
  ...
  ngOnInit() {
    ...
    const challenge = this.auth.getPublicChallengeParameters().challenge;
    ...
    // If all fields are entered, submit the form
    this.allSubscriptions.add(
      this.digit6.valueChanges
        .pipe(
          tap(() => {
            [
              this.digit1,
              ...
              this.digit5,
              this.digit6,
            ].every((control) => control.value) && this.submit();
          })
        )
        .subscribe()
    );
  ...
  public async submit() {
    try {
      ...
      const answer = [1, 2, 3, 4, 5, 6]
        ...
        .join("");
      const loginSucceeded = await this.auth.provideSecretLoginCode(answer);
      if (loginSucceeded) {
